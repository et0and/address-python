...
class TestChallenge:
    ...
    def test_method_retrieve(self, client: Address) -> None:
        challenge = client.challenge.retrieve()
        assert_matches_type(ChallengeRetrieveResponse, challenge, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    def test_raw_response_retrieve(self, client: Address) -> None:
        response = client.challenge.with_raw_response.retrieve()
    ...
    def test_streaming_response_retrieve(self, client: Address) -> None:
        with client.challenge.with_streaming_response.retrieve() as response:
...
class TestAsyncChallenge:
    ...
    async def test_method_retrieve(self, async_client: AsyncAddress) -> None:
        challenge = await async_client.challenge.retrieve()
        assert_matches_type(ChallengeRetrieveResponse, challenge, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    async def test_raw_response_retrieve(self, async_client: AsyncAddress) -> None:
        response = await async_client.challenge.with_raw_response.retrieve()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        challenge = await response.parse()
        assert_matches_type(ChallengeRetrieveResponse, challenge, path=["response"])

    @pytest.mark.skip(reason="Mock server tests are disabled")
    @parametrize
    async def test_streaming_response_retrieve(self, async_client: AsyncAddress) -> None:
        async with async_client.challenge.with_streaming_response.retrieve() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
